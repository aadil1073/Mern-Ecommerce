    ...
    <div className="d-flex flex-row align-items-start">
      ...
      Order Number : {order._id} | Date of Order : {format(new Date(order.createdAt), 'dd MMM yyyy')}
        ...
        <div className="col-12">
            <p><strong>Order Total: </strong>{order.total_cost-order.total_cost*order.discount_in_percent/100+order.shipping_charges}  |  
            <strong> Address: </strong> {order.shipping_address.first_name} {order.shipping_address.last_name} {order.shipping_address.phone} {order.shipping_address.address1} {order.shipping_address.state} {order.shipping_address.pincode}</p>
          </div>  
      </div>
    </div>
  </div>
